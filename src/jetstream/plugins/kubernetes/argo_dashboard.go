...
func (k *KubernetesSpecification) argoDashboardProxy(c echo.Context) error {
	...
	target := fmt.Sprintf("%s/api/v1/namespaces/argocd/services/https:argocd-server:https/proxy/%s", apiEndpoint, path)
	...
	if len(path) == 0 {
		log.Info("Looks like we are argo dash root! we will rewrite the html :D")
		tr, _ := rest.TransportFor(config)
		...
		log.Info("auth: " + config.BearerToken)
		...
		for scanner.Scan() {
			...
			if strings.HasPrefix(line, "    <base") {
				sb.WriteString(fmt.Sprintf("    <base href=\"%s\">", prefix))
			} else {
				sb.WriteString(line + "\n")
			}
		}
		...
		_, _ = c.Response().Write([]byte(sb.String()))
	} else {
...
func (k *KubernetesSpecification) argoDashboardStatus(c echo.Context) error {
	...
	pod, err := k.getKubeDashboardPod(c, "app%3Dargocd-server")
	if err != nil {
		pod, err = k.getKubeDashboardPod(c, "k8s-app%3Dargocd-server")
	}
